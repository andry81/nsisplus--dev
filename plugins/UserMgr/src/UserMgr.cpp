...
// Andrey Dibrov (andry at inbox dot ru): to set current locale
NSISFunction(SetLocale)
{
	EXDLL_INIT();
	extra->RegisterPluginCallback(g_hInstance, PluginCallback);
	{
		...
		setlocale(atoi(id), locale);
	}
}

// Andrey Dibrov (andry at inbox dot ru): to get current locale
NSISFunction(GetLocale)
{
	EXDLL_INIT();
	extra->RegisterPluginCallback(g_hInstance, PluginCallback);
	{
		...
		locale = setlocale(atoi(id), NULL);
		if (locale) {
			pushstring(locale);
		}
		else {
			sprintf(id, "ERROR %d", errno);
			pushstring(id);
		}
	}
}
